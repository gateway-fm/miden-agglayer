...
async fn publish_claim_internal(
    ...
    // Cantina MA#21 — NO GER-propagation wait here any more.
    //
    // The CLAIM note's FPI calls `assert_valid_ger`, which checks the bridge
    // account's GER storage; submitting a CLAIM before the GER is stored would
    // fail. Previously this site slept up to 5×3s = 15s on EVERY claim, giving
    // the NTX builder time to consume the UpdateGerNote into bridge storage.
    //
    // That wait was redundant and is now removed at the root: `ger.rs::
    // insert_ger` only returns once the bridge has actually CONSUMED the
    // UpdateGerNote (`NoteFilter::Consumed`) and the synthetic GER state is
    // committed. A GER that aggkit has marked injected is therefore already in
    // bridge storage — the bridge should only ever issue claims for GERs that
    // are already injected, so the CLAIM path carries no timing dependency on
    // the NTX builder and the sleep loop is gone entirely.
