...
pub async fn admin_register_faucet(
    ...
    // Save to store. On repair (`replace`), swap the row for this origin — the
    // freshly-deployed faucet has a new faucet_id, so a plain upsert-by-faucet_id
    // would collide with the (origin_address, origin_network) unique index.
