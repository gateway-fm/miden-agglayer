...
log "Writing .env..."
# Generate a fresh admin API key per setup. Used by the proxy's R1
# fail-closed admin auth and by e2e scripts that hit admin_* methods.
# Random per-setup so the value never leaks into any committed file.
