...
test-scripts: ## Syntax-check + run the shell guard test harnesses (no docker needed)
	...
	python3 scripts/test-e2e-funding-readiness.py
...
.PHONY: e2e-cantina12-getlogs-returns-all
e2e-cantina12-getlogs-returns-all: e2e-up ## Spin up stack + assert eth_getLogs returns ALL matches, never truncates (Cantina #12)
	$(COMPOSE_ENV) ./scripts/e2e-cantina12-getlogs-returns-all.sh
...
.PHONY: e2e-cantina10
e2e-cantina10: e2e-up ## Spin up stack + Cantina #10 concurrent first-claim faucet registration test
