...
pub(crate) async fn fetch_public_account_inputs(
    ...
    // VENDOR PATCH (gateway-fm): always fetch the vault for foreign-account
    // inputs. The upstream `VaultFetch::IfChangedFrom(local_root)` bandwidth
    // optimization is unsound here: when the node sees the vault UNCHANGED
    // from the caller's root it omits the assets from the response, but
    // `account_proof_into_inputs` unconditionally rebuilds the vault from the
    // returned asset list — producing an EMPTY vault and therefore a wrong
    // account commitment whenever the foreign account's vault is non-empty
    // and locally tracked. The transaction kernel then rejects every attempt
    // with `validate_active_foreign_account`'s "commitment of the foreign
    // account in the advice provider does not match the commitment in the
    // account tree" — deterministically, until the vault changes again.
    // Live-observed: the agglayer bridge account wedged ALL GER injections
    // (341 and 409+ consecutive identical failures on two independent stacks)
    // from the moment a native-token bridge-out locked the first asset into
    // the bridge vault. Correctness over bandwidth: fetch the assets always.
    // Remove this patch when upstream merges the unchanged-vault local-asset
    // reuse (or drops the optimization) in miden-client.
    let vault = VaultFetch::Always;
