...
impl NodeRpcClient for GrpcClient {
    ...
    async fn submit_proven_transaction(
        ...
        // idempotent — it consumes notes and mutates chain state — so it must not
        // go through `call_with_retry`.
        //
        // `call_with_retry` re-sends on `ResourceExhausted | Unavailable` (see
        // retry.rs::is_retryable), up to DEFAULT_MAX_RETRIES. If the node ACCEPTS
        // the transaction and the response is then lost (connection reset,
        // deadline, node bounce), tonic surfaces `Unavailable` and the same
        // proven transaction is silently re-sent. The retry reaches a node that
        // has already spent the nullifier, so it answers with an APPLICATION
        // -level rejection — and only that second error reaches the caller.
        //
        // That is the damaging part. A caller cannot reason about it: the
        // determinate/indeterminate distinction every safe caller depends on is
        //   * transport failure, no endpoint_error  -> node may have processed it
        //   * node answered with a rejection        -> it processed and refused
        // and the retry silently falsifies the second premise. Our bridge-out
        // tool concluded "nothing landed", minted a NEW B2AGG note, and bridged
        // the same assets twice. See `submit_is_indeterminate` in
        // src/bin/bridge_out_tool.rs, which is the caller-side backstop for
        // exactly this.
        //
        // Every OTHER endpoint keeps its retries: they are reads, where a re-send
        // is free. Remove this patch when upstream makes submit non-retried, makes
        // retryability per-endpoint, or makes submit idempotent server-side.
        // Upstream tracking: https://github.com/0xMiden/rust-sdk/issues/2441
        // (fix proposed in https://github.com/0xMiden/rust-sdk/pull/2442).
        let rpc_api = self.ensure_connected().await?;
        let api_response = {
    ...
    async fn submit_proven_batch(
        ...
        // VENDOR PATCH (miden-agglayer): same single-attempt rule as
        // submit_proven_transaction above — committing a proven batch mutates
        // chain state (it may execute input notes), so a re-send after a lost
        // response is not idempotent either. Keep this failure indeterminate
        // instead of retrying into an application-level rejection.
        let rpc_api = self.ensure_connected().await?;
        let api_response = {
            let request = request.clone();
            let mut rpc_api = rpc_api;
            rpc_api
                ...
                .await
                .map_err(|status| {
                    self.rpc_error_from_status(RpcEndpoint::SubmitProvenBatch, status)
                })?
        };
